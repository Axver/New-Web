<div id='produk_all'>

<!-- Zinc Bronze -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Dewasa 20 Hari Peninggi Badan Kalsium Zinc Tiens Tianshi Alami Meninggikan Badan (Paket Bronze)</b>
<strike style='color:red'><b>Rp 650.000</b></strike>
<h3><b>Rp 595.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Zinc Paket Bronze')">Beli</button>
</div>
</div>

<!-- Zinc Silver -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Dewasa 30 Hari Kalsium Zinc Tiens Tianshi Herbal Alami Meninggikan Badan (Paket Silver)
Rp 960,000.00</b>
<strike style='color:red'><b>Rp 1.100.000</b></strike>
<h3><b>Rp 960.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Zinc Paket Silver')">Beli</button>
</div>
</div>

<!-- Zinc Silver -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Dewasa 30 Hari Double Dosis Kalsium Zinc Tiens Tianshi Meninggikan Badan (Paket Gold)</b>
<strike style='color:red'><b>Rp 1.840.000</b></strike>
<h3><b>Rp 1.650.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Zinc Paket Gold')">Beli</button>
</div>
</div>


<!-- Zinc Biasa -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Dewasa 10 Hari Kalsium Zinc Tiens Tianshi Herbal Alami Meninggikan Badan (Paket Biasa)</b>
<strike style='color:red'><b>Rp 410.000</b></strike>
<h3><b>Rp 365.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Zinc Paket Biasa')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Anak 10 Hari Kalsium Zinc Tiens Tianshi Herbal Meninggikan Badan</b>
<strike style='color:red'><b>Rp 420.000</b></strike>
<h3><b>Rp 378.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Peninggi Badan Anak 10 Hari Kalsium Zinc Tiens Tianshi Herbal Meninggikan Badan')">Beli</button>
</div>
</div>

<!-- Zinc Biasa -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Wajah Spirulina 100 Kapsul | Wajah Kencang dan Bersih
</b>
<strike style='color:red'><b>Rp 408.000</b></strike>
<h3><b>Rp 343.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Wajah Spirulina 100 Kapsul Wajah Kencang dan Bersih')">Beli</button>
</div>
</div>

<!-- Zinc Biasa -->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Spirulina Tiens 50 Kapsul Asli | Wajah Kencang dan Bebas Jerawat
</b>
<strike style='color:red'><b>Rp 230.000</b></strike>
<h3><b>Rp 185.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Wajah Spirulina 50 Kapsul Wajah Kencang dan Bersih')">Beli</button>
</div>
</div>


<!-- Chitosen-->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Chitosan Tiens 10 Kapsul | Wajah Bebas Berminyak dan Kencang
</b>
<strike style='color:red'><b>Rp 100.000</b></strike>
<h3><b>Rp 65.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Chitosan Tiens 10 Kapsul')">Beli</button>
</div>
</div>

<!-- Spirulina-->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Spirulina Tiens 25 Kapsul | Wajah Cerah Kencang Putih Alami Herbal
</b>
<strike style='color:red'><b>Rp 140.000</b></strike>
<h3><b>Rp 110.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Masker Spirulina Tiens 25 Kapsul')">Beli</button>
</div>
</div>


<!-- Stamina-->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Stamina Pria Dewasa Tiens | Tianshi Vitalitas Pria Dewasa Menguatkan Ginjal
</b>
<strike style='color:red'><b>Rp 700.000</b></strike>
<h3><b>Rp 605.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/muncord.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Stamina Pria Dewasa Tiens')">Beli</button>
</div>
</div>


<!-- Stamina-->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Penggemuk Badan Spirulina Zinc Tiens | Tianshi Menggemukkan Badan Alami
</b>
<strike style='color:red'><b>Rp 520.000</b></strike>
<h3><b>Rp 478.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc_spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Penggemuk Badan Spirulina Zinc Tiens')">Beli</button>
</div>
</div>

<!--Kecantikan-->
<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Kecantikan Wajah dan Kulit Tiens Vitaline + Masker Chitosan 25 Kapsul Tianshi
</b>
<strike style='color:red'><b>Rp 497.800</b></strike>
<h3><b>Rp 430.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/vitaline_chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Kecantikan Wajah dan Kulit Tiens Vitaline + Masker Chitosan 25 Kapsul Tianshi')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Chitosan Tiens 25 Kapsul | Wajah Bebas Berminyak dan Kencang
</b>
<strike style='color:red'><b>Rp 140.000</b></strike>
<h3><b>Rp 120.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Kecantikan Wajah dan Kulit Tiens Vitaline + Masker Chitosan 25 Kapsul Tianshi')">Beli</button>
</div>
</div>


<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Kecantikan Wajah dan Kulit Tiens Vitaline + Masker Spirulina 25 Kapsul Tianshi
</b>
<strike style='color:red'><b>Rp 470.000</b></strike>
<h3><b>Rp 420,000.00</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/vitaline_spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Kecantikan Wajah dan Kulit Tiens Vitaline + Masker Spirulina 25 Kapsul Tianshi')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Spirulina Tiens 10 Kapsul | Wajah Cerah, Kencang & Putih Alami
</b>
<strike style='color:red'><b>Rp 63.000</b></strike>
<h3><b>Rp 55,000.00</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Spirulina Tiens 10 Kapsul')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Pemutih Kulit & Wajah Tiens | Vitaline + Masker Spirulina
</b>
<strike style='color:red'><b>Rp 470.000</b></strike>
<h3><b>Rp 420.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/vitaline_spirulina.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Pemutih Kulit & Wajah Tiens | Vitaline + Masker Spirulina')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Vitamin Mata Minus Hemat Renuves dan Vitaline Tiens | Mata Sehat
</b>
<strike style='color:red'><b>Rp 900.000</b></strike>
<h3><b>Rp 821.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/renuves_vitaline.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Vitamin Mata Minus Hemat Renuves dan Vitaline Tiens | Mata Sehat')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Vitamin Mata Minus Komplit Renuves, Vitaline dan Jiang Zhi Tea Tiens | Mata Sehat
</b>
<strike style='color:red'><b>Rp 1.001.000</b></strike>
<h3><b>Rp 951.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/renuves_vitaline_jiang.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Vitamin Mata Minus Komplit Renuves, Vitaline dan Jiang Zhi Tea Tiens')">Beli</button>
</div>
</div>

<!-- Pelangsing -->

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Pelangsing Badan Tiens | Diet Tepat | Melangsingkan Badan | Tianshi
</b>
<strike style='color:red'><b>Rp 910.000</b></strike>
<h3><b>Rp 814,000.00</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/jiang_chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Pelangsing Badan Tiens Lengkap')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Pelangsing Hemat Chitosan dan Jiang Zhi Tea Tiens Tianshi Melangsingkan Badan Alami
</b>
<strike style='color:red'><b>Rp 650.000</b></strike>
<h3><b>Rp 599.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/jiang_chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Pelangsing Badan Tiens Hemat')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Masker Wajah Chitosan 100 Kapsul | Wajah Bebas Berminyak dan Jerawat
</b>
<strike style='color:red'><b>Rp 450.000</b></strike>
<h3><b>Rp 429.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/jiang_chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Masker Wajah Chitosan 100 Kapsul | Wajah Bebas Berminyak dan Jerawat')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Peninggi Badan Anak 30 Hari Kalsium Zinc Tiens Tianshi Herbal Meninggikan Badan
</b>
<strike style='color:red'><b>Rp 1.100.000</b></strike>
<h3><b>Rp 999.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Peninggi Badan Anak 30 Hari Kalsium Zinc Tiens Tianshi Herbal Meninggikan Badan')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Detox Peluntur Lemak Chitosan dan Jiang Zhi Tea Tiens | Tianshi Membuang Lemak Jahat
</b>
<strike style='color:red'><b>Rp 602.000</b></strike>
<h3><b>Rp 559.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/jiang_chitosen.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Detox Peluntur Lemak Chitosan dan Jiang Zhi Tea Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Diabetes Komplit Tiens | Tianshi Chitosan Kalsium Shutang dan Diacont
</b>
<strike style='color:red'><b>Rp 1.200.000</b></strike>
<h3><b>Rp 1.101.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/chitosen_shutang.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Diabetes Komplit Tiens | Tianshi Chitosan Kalsium Shutang dan Diacont')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Program Kehamilan dan Kesuburan Wanita Kalsium Zinc Vitaline Tiens | Tianshi
</b>
<strike style='color:red'><b>Rp 710.000</b></strike>
<h3><b>Rp 675.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/zinc_vitaline.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Program Kehamilan dan Kesuburan Wanita Kalsium Zinc Vitaline Tiens | Tianshi')">Beli</button>
</div>
</div>


<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Diabetes Hemat Kalsium Shutang dan Chitosan Tiens | Tianshi Mengatasi Kencing Manis
</b>
<strike style='color:red'><b>Rp 800.000</b></strike>
<h3><b>Rp 711.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/chitosen_shutang.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Diabetes Hemat Kalsium Shutang dan Chitosan Tiens | Tianshi Mengatasi Kencing Manis')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Stroke Dan Penyumbatan Pembuluh Darah Tiens
</b>
<strike style='color:red'><b>Rp 2.000.000</b></strike>
<h3><b>Rp 1.524.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/stroke.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Stroke Dan Penyumbatan Pembuluh Darah Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Tipes Thypus Tiens
</b>
<strike style='color:red'><b>Rp 1.956.000</b></strike>
<h3><b>Rp 1.043.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/tipus.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Tipes Thypus Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket TBC Tiens
</b>
<strike style='color:red'><b>Rp 2.111.000</b></strike>
<h3><b>Rp 1.554.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/tipus.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket TBC Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Radang Paru ISPA Tiens
</b>
<strike style='color:red'><b>Rp 2.200.000</b></strike>
<h3><b>Rp 1.640.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/ispa.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Radang Paru ISPA Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Parkinson Syaraf Otak Tiens
</b>
<strike style='color:red'><b>Rp 1.200.000</b></strike>
<h3><b>Rp 1.010.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/parkinson.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Parkinson Syaraf Otak Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Tekanan Darah Rendah Atasi Hipotensi Tiens
</b>
<strike style='color:red'><b>Rp 1.110.000</b></strike>
<h3><b>Rp 1.043.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/darah_rendah.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Tekanan Darah Rendah Atasi Hipotensi Tiens')">Beli</button>
</div>
</div>

<div class='panel panel-info'>
<div class='panel-head'>
<b>Paket Susah Tidur Insomnia Herbal Tiens
</b>
<strike style='color:red'><b>Rp 950.000</b></strike>
<h3><b>Rp 871.000</b></h3>
</div>

<div class='panel-body'>
<div class="thumbnail">
      
        <img src="image/produk/insomnia.jpg" alt="Nature" style="width:100%">
      
</div>


<button class='btn btn-primary' onclick="beliPaket('Paket Susah Tidur Insomnia Herbal Tiens')">Beli</button>
</div>
</div>

</div>